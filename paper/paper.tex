...
a requirement. A $k$-clique (or sometimes $n$-clique or $s$-clique) is a relaxed form of clique,
where instead of requiring each pair of vertices to be directly adjacent, we only require that they
be connected by a path of length at most $k$ \cite{Luce:1950} \footnote{In an unfortunate clash of
notation, the name ``$k$-clique problem'' is sometimes used elsewhere to refer the decision version
of the clique problem, to distinguish it from the maximum clique problem.}.  Thus a $1$-clique
\emph{is} a clique, a $2$-clique may be thought of as ``a group of people, all of whom either know
each other or have a mutual acquaintance'', and so on. We illustrate this in
\cref{figure:k-cliques}.  Determining the size of a maximum $k$-clique is \NP-hard for any fixed $k$
\cite{Bourjolly:2002}.
...
graphs. Where timing results are reported, the experiments were run on a machine with Intel E5-2687W
v3 processors, and single-threaded runtimes are given. The time taken to read in the graph from a
...
unsolvable within an hour (plus a sixth when the domination rule was not used), and half of the
