...
\begin{figure}[b] % first page please
    \centering
    \begin{tikzpicture}
        % \input{figure-k-clique-1}
    \end{tikzpicture}\hspace{4em}\begin{tikzpicture}
        % \input{figure-k-clique-2}
    \end{tikzpicture}
...
\begin{algorithm}\DontPrintSemicolon
    ...
    \nl $\FuncSty{maximumKClique}$ :: (Graph $G$, Integer $k$) $\rightarrow$ Vertex Set \;
    \nl \Begin{
    ...
    \nl \Begin{
        ...
        \nl \For{$i$ $\gets$ $|P|$ $\KwSty{downto}$ 1\mclabel{righttoleft}}{
            ...
            \nl \tikzmark{cdom1}\If{\textnormal{$\vrej$ $\ne$ $\kwunset$}}{
                \nl $P$ $\gets$ $P$ $\setminus$ $\{ w \in \vertexset(G) :
                \neighbourhood_G(w) - \vrej \subseteq \neighbourhood_G(\vrej) - w \}$
                \tikzmark{cdom2} \mclabel{dominated} \;
            }
    ...
    \caption{An algorithm for the maximum $k$-clique problem.}
...
\begin{figure}[p] % these two on the same page please
    \centering
