...
bounded breadth-first search: we refer to \citeauthor{Chang:2013} \shortcite{Chang:2013} for how to
...
The key step in \citeauthor{Tomita:2010}'s algorithms is to produce a constructive colouring, which
is used in a clever way.  The $\colourOrder$ routine does not just return the number of colours
used. Instead, it returns a pair of arrays, $\order$ and $\bounds$. The $\order$ array contains
vertices, in the order in which they were coloured. The $i$th entry of the $\bounds$ array contains
the colour number used for the $i$th vertex in $\order$. We illustrate this in
\cref{figure:colours}. Crucially, $\bounds$ is non-decreasing (i.e.\ $\bounds[i+1] \ge \bounds[i]$),
and we may colour the subgraph induced by the first $i$ vertices of $\order$ using $\bounds[i]$
colours.
...
\begin{table}
    \tiny\setlength{\tabcolsep}{6pt} % carefully tuned!
    \centering
...
By comparing these results with the $k$-club results of Chang et al.\ \shortcite{Chang:2013}, we see that
...
\shortcite{Wotzlaw:2014} confirmed privately that the $k$-clique and $k$-club numbers are the same here
...
\begin{table}
    \tiny\setlength{\tabcolsep}{4pt} % carefully tuned!
    \centering
    \begin{tabular}{l c rr rr rr}
        \toprule
        & & & & \multicolumn{2}{c}{Unmodified} & \multicolumn{2}{c}{With Domination} \\
...
\begin{table}
    \tiny\setlength{\tabcolsep}{5pt} % carefully tuned!
    \centering
    \begin{tabular}{l c rr rr rr}
        \toprule
        & & & & \multicolumn{2}{c}{Unmodified} & \multicolumn{2}{c}{With Domination} \\
...
\begin{table}
    \tiny\setlength{\tabcolsep}{4.6pt} % carefully tuned!
    \centering
    \begin{tabular}{l c rr rr rr}
        \toprule
...
instances, and improved the bounds on ?? further instances with 12 hours of search. However, to get
sufficient work balance, we had to allow for work splitting to depth 10 rather than depth 3. A close
inspection of the search patterns showed that simpler static decomposition approaches
...
``cond-mat'' and $k=4$, and on ?? of the graphs where the incumbent was improved, it was improved
after less time than the sequential runs took. This is because the work splitting mechanism we used
explicitly diversifies at the top of search first, where branching heuristics are least likely to be
correct, which leads to an initial incumbent being found faster---this is in line with recent
observations that tailored work stealing should be favoured over randomised work stealing for
...
\cite{DBLP:conf/waw/PattabiramanPGLC13,DBLP:journals/cor/SegundoLP16}, and we were considering
...
eight of the problem instances we considered, and many others would have taken much longer. However,
...
level of task granularity than usual.  We suspect further progress could be made, for example, by
tailoring the initial vertex ordering based upon what we know about the graphs, or by increasing the
number of recursive calls per second by making the colouring stage cheaper, for example by reusing
colour classes \cite{DBLP:conf/lion/NikolaevBS15}.
